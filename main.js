...
function initLangSwitch() {
    ...
    function setLang(lang) {
        nodes.forEach((el) => {
            ...
            el.innerHTML = lang === 'de' && de !== null ? de : ((_a = en.get(el)) !== null && _a !== void 0 ? _a : '');
        });
    ...
    const initial = (navigator.language || 'en').toLowerCase().indexOf('de') === 0 ? 'de' : 'en';
    setLang(initial);
}
// ---- movable sprite demo ----
...
async function initGameSourceViewer() {
    ...
    if (typeof hljs !== 'undefined') {
        ...
        catch ( /* highlighting is best-effort */_a) { /* highlighting is best-effort */ }
    }
    if (copyBtn) {
...
function initPlayableGame() {
    ...
    document.querySelectorAll('.game-pad button').forEach((btn) => {
    ...
    function update() {
        ...
        if (g.got.length === g.coins.length)
            g.won = true;
    }
    function draw() {
